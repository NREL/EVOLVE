...
def get_charge_need_func():
    ...
    def get_charge_need_pref(day: datetime.date):
        ...
        return ChargingNeedPreference(target=ChargTargetModel.time_based, desired_duration=9000)
...
def random_electric_vehicles_builder_v1(
    ...
    for item in vehicles:
        for id_ in range(item.numberOfEV):
            ...
            prefers_special_hour = np.random.choice(
                        [True, False],
                        p=[
                            item.pctVehiclesForPreferredHour / 100,
                            1 - item.pctVehiclesForPreferredHour / 100,
                        ],
                    )
            electric_vehicles.append(
                EVModel(
                    ...
                    station_category_order=random.sample(station_types, len(station_types)),
                    preferred_charge_hour=item.preferredHour if prefers_special_hour else None
                )
            )
...
def random_charging_stations_builder_v1(
    ...
    for item in stations:
        for id_ in range(item.numberOfStations):
            ...
            stations_type_mapper.append(EVTypeMapper(id=station_id, type_=item.stationCategoryName))
            charging_stations.append(
                ChargStationModel(
                    id=station_id,
                    num_of_slots=np.random.randint(*item.numberOfSlots, 1)[0],
...
class LargeScaleEVSimulatorV1:
    ...
    def __init__(self, vehicles: List[ElectricVehiclesForm], stations: List[ChargingStationsForm]):
    ...
    def aggregate_vehicle_results(self, trans_model: TransportationModel) -> pl.DataFrame:
        """Internal method to aggregate results for various vehicle types."""

        vehicle_df = trans_model.get_ev_total_charging_df()
        aggregated_data = {"timestamp": vehicle_df["Timestamps"]}

        for vehicle_type in self.vehicle_types:
            filtered_vehicles = [el.id for el in self.vehicle_mapper if el.type_ == vehicle_type]
            aggregated_data[vehicle_type] = vehicle_df[filtered_vehicles].sum(axis=1)

        return pl.DataFrame(aggregated_data)

    def aggregate_station_results(self, trans_model: TransportationModel) -> pl.DataFrame:
        ...
        for station_type in self.station_types:
            filtered_stations = [el.id for el in self.station_mapper if el.type_ == station_type]
