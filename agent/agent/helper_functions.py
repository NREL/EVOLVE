...
def populate_sliced_category(df: polars.DataFrame):
    """Populate dataframe with sliced category."""
...
def upsample_interpolate_df(df: polars.DataFrame, resolution: int):
    """Upsampling dataframe using linear interpolation."""
...
def upsample_staircase_df(df: polars.DataFrame, resolution: int):
    """Upsampling dataframe using staircase fill."""
...
def filter_by_date(df: polars.DataFrame, start_date: datetime.date, end_date: datetime.date):
    ...
    return df.filter((polars.col("timestamp") > start_date) & (polars.col("timestamp") < end_date))
...
def get_file_name_from_id(id: int):
    ...
    with PostGresDB(DB_CONFIG) as cursor:
        cursor.execute(
            """select users.username, timeseriesdata.filename, timeseriesdata.resolution_min 
                from timeseriesdata 
                inner join users 
