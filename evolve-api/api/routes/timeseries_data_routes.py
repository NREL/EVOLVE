...
from api import timeseries_data
from api.custom_models import TimeSeriesDataResponseModel, SharedUserInfoModel
...
async def get_data_for_download(
    ...
    ts_data = await models.TimeseriesData.get(id=id).prefetch_related(
        "user__user"
    )
    ...
    file_path = (
        Path(DATA_PATH) / ts_data.user.username / "timeseries_data" / file_name
    )
...
async def upload_timeseries_data(
    ...
    metadata_pydantic = timeseries_data.TSFormInput.model_validate_json(
        metadata
    )
    # Although it would be ideal to run this upload
    # in background task, recent version of FASTapi closes connection
    # to spooled file once response is returned.

    await timeseries_data.handle_timeseries_data_upload(
