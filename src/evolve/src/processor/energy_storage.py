...
def compute_es_energy_metric(battery_power_df:polars.DataFrame, resolution:int):
    ...
    for column in df.columns:
        if column not in ['timestamp', 'category']:
            ...
            charging_df = df.filter(polars.col(column)<0).groupby("category" \
                ).agg(polars.col(column).sum()).with_column(
                (polars.col(column)*(-resolution/60)).alias(column)
            ).select([column, 'category'])

            discharging_df = df.filter(polars.col(column)>0).groupby("category" \
                ).agg(polars.col(column).sum()).with_column(
                (polars.col(column)*(resolution/60)).alias(column)
            ).select([column, 'category'])
            ...
            if not len(charging_new_df):
                ...
                charging_new_df = charging_df.join(charging_new_df, 
                    on='category', how='left')
                discharging_new_df = discharging_df.join(discharging_new_df, 
                    on='category', how='left')
    return (charging_new_df, discharging_new_df)


def compute_energy_storage_metrics(battery_power_df:polars.DataFrame, 
    resolution:int, base_path:str ):
    
    charging_energy_df, discharging_energy_df \
        = compute_es_energy_metric(battery_power_df, resolution)

    charging_energy_df.write_csv(base_path / 'es_charging_energy_metrics.csv')
    discharging_energy_df.write_csv(base_path / 'es_discharging_energy_metrics.csv')
...
def process_energy_storage(
    ...
    battery_power_dict = {bname: bdict['battery_power'] \
    ...
    battery_soc_dict = {bname: bdict['battery_soc'] \
        for bname, bdict in battery_output.items()}
